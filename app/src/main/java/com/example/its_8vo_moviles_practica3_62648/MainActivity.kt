...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val operations = mapOf(
            ...
            R.id.btnPow to "^", R.id.btnSquareRot to "√",
            R.id.btnPercent to "%"
        )
    ...
    private fun calculateResult() {
        ...
        if (firstOperand != null && secondOperand != null && operator != null) {
            val result = when (operator) {
                ...
                "^" -> firstOperand!!.pow(secondOperand) // agregado para la practica
                "%" -> (firstOperand!! / 100) * secondOperand // agregado para la practica
                else -> null
            }
            ...
            firstOperand = null
            operator = null
        }else if(firstOperand != null && operator != null){ // agregado para la practica
            var result = when (operator){
                "√" -> firstOperand!!.pow(0.5)
                else -> null
            }
            txtResult.text = result?.toString() ?: "Error"
            currentInput = result?.toString() ?: ""
